...
(() => {
  ...
  // src/model/expiry.ts
  function rows(items, now) {
    return items.filter((i) => i.expires !== void 0 && (i.kind === "pki" || i.kind === "dynamic")).map((item) => {
      const end = item.expires;
      const tone = end <= now || item.tone === "error" ? "error" : item.tone === "warn" ? "warn" : "ok";
      const life = item.issuedAt !== void 0 ? end - item.issuedAt : void 0;
      return {
        item,
        start: item.issuedAt,
        end,
        ...
        used: life && life > 0 ? Math.min(1, Math.max(0, (now - item.issuedAt) / life)) : void 0
      };
  ...
  function heading(current, title, note, counts = {}) {
  ...
  function block(title, note, ...children) {
    return el("section", { class: "block" }, el("h2", {}, title), note ? el("p", { class: "note" }, note) : null, ...children.filter((c) => !!c));
  }
  ...
  start("page", async () => {
    ...
    const stop = every(
      ...
      async () => {
        ...
        const print = fingerprint([snap, Math.floor(Date.now() / 6e4)]);
  ...
  var ROW_H = 38;
  var HEAD = 28;
  function timeline(list, axis, now) {
    ...
    const x = (at) => nowX + position(axis, at - now) * CHART_W;
    ...
    list.forEach((r, i) => {
      ...
      if (r.renew !== void 0 && r.renew < r.end - 1e3) {
      ...
      const used = r.used !== void 0 ? ` ${Math.round(r.used * 100)}% of its life is used.` : "";
      tip.textContent = `${r.item.namespace}/${r.item.name}: expires ${moment(r.end)}${r.renew !== void 0 && r.renew < r.end ? `; the operator renews it ${relative(r.renew, now)}` : ""}.${used} ${r.item.message ?? ""}`.trim();
      g.append(tip);
      clickable(g, () => void k8sdockside.open(r.item.ref));
      svg.append(g);
    });
  ...
  function table(list, now) {
    ...
    for (const r of list) {
      const row = el(
        ...
        el("td", {}, r.start !== void 0 ? moment(r.start) : "—"),
        el("td", {}, r.renew !== void 0 && r.renew < r.end - 1e3 ? relative(r.renew, now) : "at expiry"),
